...
function buildInsurerPrompt({
  ...
  return [
    ...
    "- 제목은 하나의 고정 포맷만 반복하지 말고, 데이터상 가장 두드러진 특징에 맞춰 다양하게 생성할 것",
    "- 우선 판단할 핵심 포인트는 다음과 같다: 1위 GA 독주 여부, 상위 3개사·5개사 집중도, 1~3위 간 격차, 2~5위 접전 여부, 급등·급락 GA 존재, 최근 3개월 연속 확대 여부, 나머지 GA가 한 자릿수 점유율인지 여부",
    "- 제목은 아래 패턴 중 데이터에 맞는 것을 선택해 사용할 것",
    `  1. "{보험사명} {월} GA 생보실적 M/S…{1위 GA} {1위 점유율}% 1위, {2위 GA}·{3위 GA} 뒤이어"`,
    `  2. "{보험사명} {월} GA 생보실적 M/S…{1위 GA} 선두 유지, {상승 GA} 순위 상승·상위권 재편"`,
    `  3. "{보험사명} {월} GA 생보실적 M/S…‘{1위 GA}’ 1위 유지 속, ‘{2위 GA}·{3위 GA}’ 상위권 형성"`,
    `  4. "{보험사명} {월} GA 생보실적 M/S…상위 3개사 점유율 {합산 점유율}%, {핵심 GA1}·{핵심 GA2}·{핵심 GA3} 각축"`,
    `  5. "{보험사명} {월} GA 생보실적 M/S…상위 5개사 {합산 점유율}% 집중, {1위 GA} 1위 유지"`,
    `  6. "{보험사명} {월} GA 생보실적 M/S…{1위 GA} {1위 점유율}%, 나머지 모두 한 자릿수"`,
    `  7. "{보험사명} {월} GA 생보실적 M/S…{1위 GA} 선두, {2위 GA}·{3위 GA}·{4위 GA} 접전"`,
    `  8. "{보험사명} {월} GA 생보실적 M/S…3개월 연속 비중 키운 {1위 GA} {1위 점유율}% 1위, {2위 GA}·{3위 GA} 순"`,
    "- 예시 문구를 그대로 복사하지 말고, 실제 데이터 특징에 맞게 자연스럽게 변형할 것",
    "- 데이터상 특징이 약하면 과장하지 말고, 점유율·집중도·순위 변동 중 가장 설명력이 큰 요소 하나만 제목에 반영할 것",
    "- 제목에는 가능하면 아래 표현도 상황에 따라 활용할 것: '선두 유지', '1위 탈환', '상위권 형성', '순위 재편', '접전', '독주', '한 자릿수', '상승세', '나홀로 20%대', '상위 3개사 집중'",
    "",
    "[본문 구성]",
    ...
    `- 최근 12개월 기준 GA별 점유율 데이터: ${yearlyStructureLines}`,
    `- 최근 월별 추이:\n${trendLines}`,
...
function buildInsurerDashboardState(
  ...
  const rankedRows = (!isAllInsurersView && otherRanked.length > 0)
    ? [
        ...visibleRanked,
        {
